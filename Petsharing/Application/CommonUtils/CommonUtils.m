...
- (NSString *) getFullResPhotoUrl:(NSString *)url withType:(NSString *)type {
    if([type isEqualToString:@"res"] && [url rangeOfString:MEDIA_POST_PHOTO_SELF_DOMAIN_PREFIX].location != NSNotFound) {
        return [MEDIA_URL_POST_PHOTO stringByAppendingString:[url substringFromIndex:[MEDIA_POST_PHOTO_SELF_DOMAIN_PREFIX length]]];
    } else {
        return url;
    }
}
...
- (BOOL)isEmptyString:(NSString *)str {
    ...
    if([trimmedString isEqualToString:@""] || [trimmedString isEqualToString:@"0"] ) {
        return YES;
    } else {
        return NO;
    }
...
- (NSDate *)convertStringToDate:(NSString *)dateStr {
    ...
    [dateFormat setDateFormat:PetDateFormatStr];
...
- (void)setCustomUserPhoto:(UIImageView *)imageView byPhotoUrl:(NSString *)photoUrl {
    [commonUtils setImageViewAFNetworking:imageView withImageUrl:[commonUtils getFullPhotoUrl:photoUrl withType:@"user"] withPlaceholderImage:[UIImage imageNamed:MEDIA_DEFAULT_USER_PHOTO_URL]];
...
- (void) multileButtonActions:(UIButton *)sender inButtons:(NSArray *)btns{
    for (UIButton* btn in btns) {
        if (btn == sender) {
            [btn setBackgroundColor:appController.appMainColor];
            [btn setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
        } else {
            [btn setBackgroundColor:[UIColor clearColor]];
            [btn setTitleColor:appController.appMainColor forState:UIControlStateNormal];
        }
    }
}
...
- (void) overlayGradientView:(UIView*) view
{
    ...
    gradient.colors = [NSArray arrayWithObjects:(id)
                       [(id)RGBA(0, 51, 232, 1.0f) CGColor],
                        (id)[RGBA(0, 159, 230, 1.0f) CGColor], nil];
...
#pragma mark - ICE
- (void)changeUITextFiledPlaceHolderColor:(UITextField*)textField
                      withPlaceHolderText:(NSString*)placeholderText
                                withColor:(UIColor*) color
{
    if (SYSTEM_VERSION_LESS_THAN(@"6.0")) return;
    ...
    if ([textField respondsToSelector:@selector(setAttributedPlaceholder:)])
    {
        ...
        // Add fall-back code to set placeholder color.
    }
...
- (void)addBlurEffectToView:(UIView *)view withAlpha:(CGFloat) alpha
{
...
- (NSMutableAttributedString*)changeFontColorOfSubstring:(NSString *) initialStr
                                                toKeystr:(NSString*) keyStr
                                        withKeyFontColor:(UIColor*) keyFontColor
                                     withEventOwnerNameStr:(NSString*)eventOwnerNameStr
                                        withEventNameStr:(NSString *)eventNameStr
{
 
    
  //  NSString *initial = @"This is the text and i want to replace something and stuff and stuff";
    NSString *addMarkToKeyString = [NSString stringWithFormat:@"#%@", keyStr];
    
    // Add "#" to Keystring Appearence
    NSString *addedMarkToIntialStr = [initialStr stringByReplacingOccurrencesOfString:keyStr withString: addMarkToKeyString];
    
    NSMutableAttributedString *resultStr = [[NSMutableAttributedString alloc] initWithString:addedMarkToIntialStr];
    
    
    
    // Change KeyString Font color
    NSRegularExpression *regex = [NSRegularExpression
                                  regularExpressionWithPattern:[NSString stringWithFormat:@"(%@)",addMarkToKeyString]
                                  options:kNilOptions
                                  error:nil];
    
    NSRange range = NSMakeRange(0,addedMarkToIntialStr.length);
    
    [regex enumerateMatchesInString:addedMarkToIntialStr options:kNilOptions range:range usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
        
        NSRange subStringRange = [result rangeAtIndex:1];
        [resultStr addAttribute:NSForegroundColorAttributeName value:keyFontColor range:subStringRange];
    }];
    
    
    // Change EventOwnerName Font to Bold font
    regex = [NSRegularExpression
                                  regularExpressionWithPattern:[NSString stringWithFormat:@"(%@)",eventOwnerNameStr]
                                  options:kNilOptions
                                  error:nil];
    
    range = NSMakeRange(0,addedMarkToIntialStr.length);
    
    [regex enumerateMatchesInString:addedMarkToIntialStr options:kNilOptions range:range usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
        
        NSRange subStringRange = [result rangeAtIndex:1];
        [resultStr addAttribute:NSFontAttributeName value:[UIFont fontWithName:@"Lato-Regular" size:16] range:subStringRange];
    }];
    ...
    // Change EventOwnerName Font to Bold font
    regex = [NSRegularExpression
             regularExpressionWithPattern:[NSString stringWithFormat:@"(%@)",eventNameStr]
             options:kNilOptions
             error:nil];
...
// Add Line in the middle of string (Ice / End Calendar date)
- (NSMutableAttributedString*) strikeString:(NSString*)str
{
...
// Two font in one label (Right Menu/ Canlender Title)
- (NSMutableAttributedString *) integrate2AttirbuteString:(NSString*)str1
                                           withAttribute1:(NSDictionary*)attribute1
                                              withString2:(NSString*)str2
                                           withAttribute2:(NSDictionary*)attribute2
{

    NSMutableAttributedString *AattrString = [[NSMutableAttributedString alloc] initWithString:str1 attributes: attribute1];
    
    [AattrString appendAttributedString:[[NSMutableAttributedString alloc] initWithString:@" "]];

    NSMutableAttributedString *VattrString = [[NSMutableAttributedString alloc]initWithString: str2 attributes:attribute2];
...
// "Monday 03/11/14" -> "MONDAY 03/11/14" (Right Menu / Bottom TableView/Section)
- (NSString *) upperCaseWeekdaysFromDateStr:(NSString*)dateString
{
...
// Get Top Section in UITableView
- (NSInteger) topSection:(UITableView *)tableView
{
