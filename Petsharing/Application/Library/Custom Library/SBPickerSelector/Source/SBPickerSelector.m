...
+ (instancetype) pickerWithNibName:(NSString*)nibName {
    ...
    // Land
    instance.optionsToolBar.barTintColor = [UIColor blackColor];
    [instance.doneButton setTintColor:[UIColor whiteColor]];
...
- (void) showPickerOver:(UIViewController *)parent{
    ...
    UIColor *color = [UIColor colorWithRed:95/255.0f green:191/255.0f blue:237/255.0f alpha:.8f];
...
//		frame.size.height = CGRectGetHeight(self.topView.frame) + CGRectGetHeight(self.datePickerView.frame);
        frame.size.height = CGRectGetHeight(self.topView.frame) + CGRectGetHeight(self.datePickerView.frame);
		self.view.frame = frame;
...
-(CGSize) pickerSize{
	...
	if (self.pickerType == SBPickerSelectorTypeDate && self.datePickerType != SBPickerSelectorDateTypeOnlyMonthAndYear) {
		size.height = CGRectGetHeight(self.topView.frame) + CGRectGetHeight(self.datePickerView.frame);
		size.width = CGRectGetWidth(self.datePickerView.frame);
	}else{
		size.height = CGRectGetHeight(self.topView.frame) + CGRectGetHeight(self.pickerView.frame);
...
//	[self.dateOnlyMonthYearPickerView selectToday]; 
	...
	CGRect frame = self.view.frame;
	frame.size.height = CGRectGetHeight(self.topView.frame) + CGRectGetHeight(self.pickerView.frame);
	self.view.frame = frame;
